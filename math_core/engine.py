...
def clear_scratch_buffer_pool():
    ...
    try:
        from color_engine import clear_gpu_cmap_cache
        clear_gpu_cmap_cache()
    except Exception:
        pass
    if HAS_CUDA:
        try:
...
def _is_cancelled(token):
    """Checks if a cancellation token, flag, or callable has been signaled."""
    if token is None:
        return False
    if callable(token):
        try:
...
def _safe_call_strip_cb(cb, cur, tot, stage=""):
    """Safely invokes a strip progress callback with optional stage name support."""
    if cb is None:
        return
    try:
        cb(cur, tot, stage)
...
def eval_glitched_pixels_fallback(out_ary, glitched_indices, target_w, target_h, max_iter,
                                  ...
                                  cdf_lut=None, num_bins=2048, iter_map=None, cancel_token=None):
    """Shared multi-threaded arbitrary-precision fallback evaluator for glitched pixels in Strict Metric mode."""
    if len(glitched_indices) == 0 or _is_cancelled(cancel_token):
        return out_ary
...
def render_frame_cpu(target_w, target_h=None, max_iter=None, center_x_dec=None, center_y_dec=None, plot_width_dec=None,
                     ...
                     num_strips=1, strip_callback=None, cancel_token=None):
    ...
    if isinstance(target_w, RenderParameters):
        ...
        return render_frame_cpu(
            ...
            num_strips=getattr(p, 'num_strips', num_strips),
            strip_callback=getattr(p, 'strip_callback', strip_callback),
            cancel_token=getattr(p, 'cancel_token', cancel_token)
        )
    if _is_cancelled(cancel_token):
        return None
    ...
    if (use_floatexp or use_perturbation) and is_auto_rebase:
        ...
        if is_exhaustive:
            glitched_indices = np.argwhere(out_glitch > 0)
            if len(glitched_indices) > 0 and not _is_cancelled(cancel_token):
                eval_glitched_pixels_fallback(
    ...
    if _is_cancelled(cancel_token):
        return None
    if strip_callback is not None and num_strips > 1:
        _safe_call_strip_cb(strip_callback, num_strips, num_strips)
...
def _dispatch_kernel_striped(kernel, grid_x, total_grid_y, threads, stream, args, n_strips=1, callback=None, cancel_token=None):
    """Launches a CUDA kernel either monolithically or partitioned into horizontal strips with progress callbacks and cancellation checks."""
    if _is_cancelled(cancel_token):
        return False

    if n_strips <= 1 or total_grid_y <= 1:
        kernel[(grid_x, total_grid_y), threads, stream](*args, 0)
        if callback:
            _cuda_sync_stream(stream)
            callback(1, 1)
        return not _is_cancelled(cancel_token)

    strip_grid_y = (total_grid_y + n_strips - 1) // n_strips
    for s in range(n_strips):
        if _is_cancelled(cancel_token):
            return False
        by_start = s * strip_grid_y
        by_count = max(0, min(strip_grid_y, total_grid_y - by_start))
        if by_count <= 0:
            if callback:
                callback(s + 1, n_strips)
            continue
        y_offset = by_start * 16
        kernel[(grid_x, by_count), threads, stream](*args, y_offset)
        if callback:
            _cuda_sync_stream(stream)
            callback(s + 1, n_strips)
        if _is_cancelled(cancel_token):
...
def render_frame_ssaa(target_w, target_h=None, max_iter=None, center_x_dec=None, center_y_dec=None, plot_width_dec=None, cmap_name=None,
    ...
    if compute_device == "CPU" or not HAS_CUDA:
        return render_frame_cpu(
            ...
            num_strips=num_strips, strip_callback=strip_callback,
            cancel_token=cancel_token
        )
    ...
    threads = (16, 16)
    grid_x = (target_w + 15) // 16
    total_grid_y = (target_h + 15) // 16
    blocks = (grid_x, total_grid_y)
    ...
    if color_scheme_id == 11:
        ...
        if use_fp64:
            ...
            ok = _dispatch_kernel_striped(
                mandelbrot_unified_fp64_kernel, grid_x, total_grid_y, threads, stream,
                ...
                n_strips=num_strips,
                callback=lambda c, t: _safe_call_strip_cb(strip_callback, c, t, "Pass 1/2" if factor > 1 else ""),
                cancel_token=cancel_token
            )
            if not ok or _is_cancelled(cancel_token):
                return None
        elif use_dd:
            x_min_hi, x_min_lo = to_dd(center_x_dec - (plot_width_dec / Decimal(2)))
            y_max_hi, y_max_lo = to_dd(center_y_dec + (plot_height_dec / Decimal(2)))
            ...
            ok = _dispatch_kernel_striped(
                mandelbrot_unified_dd_kernel, grid_x, total_grid_y, threads, stream,
                (d_out_rgb, d_iter_map, d_dummy_edge, d_lut, target_w, target_h, max_iter,
            ...
            ok = _dispatch_kernel_striped(
                mandelbrot_unified_floatexp_kernel, grid_x, total_grid_y, threads, stream,
            ...
            ok = _dispatch_kernel_striped(
                mandelbrot_unified_perturbation_kernel, grid_x, total_grid_y, threads, stream,
                (d_out_rgb, d_glitch_out, d_iter_map, d_dummy_edge, d_lut, target_w, target_h, max_iter,
                 d_ref_re, d_ref_im, ref_len,
                 ref_px_sub, ref_py_sub, dx_sub_f, dy_sub_f,
                 ref_cx_f, ref_cy_f, factor, 0, fractal_type,
        ...
        if factor > 1:
            ...
            if use_fp64:
                ok = _dispatch_kernel_striped(
                    ssaa_resolve_hist_fp64_kernel, grid_x, total_grid_y, threads, stream,
                    (d_out_rgb, d_iter_map, d_edge_mask, d_cdf_lut, d_lut, target_w, target_h, max_iter,
                     ...
                     float(color_density), float(color_contrast)),
                    n_strips=num_strips,
                    callback=lambda c, t: _safe_call_strip_cb(strip_callback, c, t, f"Pass 2/2: SSAA {factor}x"),
                    cancel_token=cancel_token
                )
                if not ok or _is_cancelled(cancel_token):
                    return None
                ...
                ok = _dispatch_kernel_striped(
                    ssaa_resolve_hist_dd_kernel, grid_x, total_grid_y, threads, stream,
                    (d_out_rgb, d_iter_map, d_edge_mask, d_cdf_lut, d_lut, target_w, target_h, max_iter,
                     x_min_hi, x_min_lo, dx_hi, dx_lo,
                     y_max_hi, y_max_lo, dy_hi, dy_lo,
                     factor, fractal_type,
                ...
                ok = _dispatch_kernel_striped(
                    ssaa_resolve_hist_floatexp_kernel, grid_x, total_grid_y, threads, stream,
                ...
                ok = _dispatch_kernel_striped(
                    ssaa_resolve_hist_perturbation_kernel, grid_x, total_grid_y, threads, stream,
                    (d_out_rgb, d_glitch_out, d_iter_map, d_edge_mask, d_cdf_lut, d_lut, target_w, target_h, max_iter,
                     d_ref_re, d_ref_im, ref_len,
                     ref_px_sub, ref_py_sub, dx_sub_f, dy_sub_f,
                     ref_cx_f, ref_cy_f, factor, fractal_type,
        ...
        if factor == 1:
            ok = _dispatch_kernel_striped(
                ...
                (d_out_rgb, d_active_iter, d_dummy_edge, d_lut, target_w, target_h, max_iter,
                 ...
                 float(julia_cx), float(julia_cy), int(gen_n), float(gen_kr), float(gen_ki), 0,
                 float(palette_offset), float(color_density), float(color_contrast)),
                n_strips=num_strips, callback=lambda c, t: _safe_call_strip_cb(strip_callback, c, t, "Render"),
                cancel_token=cancel_token
            )
            if not ok or _is_cancelled(cancel_token):
                return None
            ...
            ok = _dispatch_kernel_striped(
                ...
                (d_out_rgb, d_iter_map, d_dummy_edge, d_lut, target_w, target_h, max_iter,
                 ...
                 float(julia_cx), float(julia_cy), int(gen_n), float(gen_kr), float(gen_ki), 0,
                 float(palette_offset), float(color_density), float(color_contrast)),
                n_strips=num_strips, callback=lambda c, t: _safe_call_strip_cb(strip_callback, c, t, "Pass 1/2: Base Grid"),
                cancel_token=cancel_token
            )
            if not ok or _is_cancelled(cancel_token):
                return None
            ...
            ok = _dispatch_kernel_striped(
                ...
                (d_out_rgb, d_iter_map, d_edge_mask, d_lut, target_w, target_h, max_iter,
                 ...
                 float(palette_offset), float(color_density), float(color_contrast)),
                n_strips=num_strips, callback=lambda c, t: _safe_call_strip_cb(strip_callback, c, t, f"Pass 2/2: SSAA {factor}x"),
                cancel_token=cancel_token
            )
            if not ok or _is_cancelled(cancel_token):
                return None
            ...
            if use_floatexp:
                ...
                if color_scheme_id == 11 and d_cdf_lut is not None:
                    ssaa_resolve_hist_floatexp_kernel[blocks, threads, stream](
                        ...
                        num_bins, float(palette_offset), float(color_density), float(color_contrast), 0
                    )
                else:
        ...
        if is_exhaustive:
            ...
            if len(glitched_indices) > 0 and not _is_cancelled(cancel_token):
                eval_glitched_pixels_fallback_cuda(
                    ...
                    d_iter_map=d_active_iter, stream_slot=stream_slot, cancel_token=cancel_token
                )
