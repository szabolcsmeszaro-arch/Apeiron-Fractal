...
* **Double-Double (DD / 106-bit Floating Point)**: Native Dekker-split arithmetic providing ~31 decimal digits of precision ($10^{-24} \le w < 10^{-12}$) without arbitrary-precision overhead.
...
* **Instant Iteration-Cache Recoloring Engine (~0.08ms GPU / ~1ms CPU)**: Switching color schemes, shifting palette phases, or adjusting density and contrast immediately re-shades the viewport from cached screen-space iteration buffers (`d_iter_buf` / `h_iter_buf`) without recalculating reference orbits, BLA leap tables, or escape iterations.
* **Over 1,500+ Curated Colormaps**: Full integration with Matplotlib and Palettable suites across 10 specialized categories (**★ Popular / Recommended**, **CartoColors**, **cmocean**, **ColorBrewer2**, **Cubehelix**, **Light & Bartlein**, **matplotlib**, **MyCarta**, **Scientific**, **Tableau**) with instant real-time search.
* **Real-Time Palette Phase Shifts**: Rotate and animate palette phase contours on the fly in real time via sidebar controls or instant hotkeys (`[` and `]`), with 60 FPS live animation playback (<kbd>Space</kbd>).
...
### 6. Export Studio
* **Ultra-High Resolution Image Export (Save PNG)**: Renders up to 16K+ PNGs with SSAA memory safety diagnostics, 16-bit HDR deep color, and exact Decimal-precision filename tags.
* **Image Zoom (Multi-Waypoint Sequence Export)**: Exports sequenced image frames along deep-zoom trajectories or smoothly interpolates arbitrary multi-point waypoint paths with Catmull-Rom splines. Features session state persistence (`fractal_seq_session.json` and **Load Session...**), resume/skip already-rendered frames, and automatic 60 FPS frame count calculation.
* **Video Zoom (Cinematic MP4 / WebM Export)**: Multi-keyframe camera fly-through paths parameterized by cumulative scale-space arc-length and logarithmic focal tracking for uniform, constant perceived optical velocity across arbitrarily many waypoints.
* **Image Mosaic (Tile Grid Export)**: Multi-tile partitioned rendering with automatic seamless stitching, memory safety diagnostics, and per-tile telemetry HUD overlays.
...
Below are actual renders produced directly with Apeiron across diverse coordinate depths and formulas:

| Burning Ship Exploration ($w = 0.18$) | Seahorse Valley Minibrot ($w = 10^{-10}$) | Ultra-Deep Perturbation ($w = 10^{-160}$) |
|:---:|:---:|:---:|
| <img src="images/fractal_image_1280x720_2xSSAA_-1.750000e+00_3.000000e-02_1.800000e-01.png" width="100%" alt="Burning Ship Fractal" /> | <img src="images/fractal_image_1280x720_2xSSAA_-7.436439e-01_1.318259e-01_1.000000e-10.png" width="100%" alt="Mandelbrot Seahorse Valley 1e-10" /> | <img src="images/fractal_image_1280x720_2xSSAA_-8.398228e-01_-2.229643e-01_1.000000e-160.png" width="100%" alt="Ultra-Deep Zoom 1e-160" /> |
| Center: `(-1.75, 0.03)` | Center: `(-0.743644, 0.131826)` | Center: `(-0.839823, -0.222964)` |
...
cd Fractura
```
*(Or clone into `Apeiron` directory)*:
```bash
git clone https://github.com/szabolcsmeszaro-arch/Fractura.git Apeiron
cd Apeiron
```
...
```bash
pip install numpy numba PyQt5 matplotlib palettable psutil imageio-ffmpeg pillow gmpy2
```

> [!NOTE]
> **GMPY2 Acceleration**: `gmpy2` provides ~10× faster arbitrary-precision reference orbit calculations for deep zooms ($w < 10^{-12}$). If `gmpy2` is not installed on your system, Apeiron automatically falls back to Python's built-in `decimal` module without any loss of precision.
...
| **Mouse Wheel Up** | **Zoom In** | Smooth geometric magnification scaled by the active **Wheel Zoom** factor (default $1.25\times$) centered precisely on your cursor position. |
| **Mouse Wheel Down** | **Zoom Out** | Smooth geometric demagnification scaled by the active **Wheel Zoom** factor (default $1.25\times$) centered precisely on your cursor position. |
...
| **Left Double-Click** | **Julia Jump / Waypoint** | When Julia HUD is visible, jumps viewport into the selected Julia set; otherwise records a flight waypoint / keyframe at the cursor coordinate. |
| **`Space`** | **Toggle Live Palette Animation** | Starts or pauses continuous 60 FPS real-time palette phase cycling (safe when typing in input boxes). |
| **`R`** | **Reset Viewport** | Resets viewport center coordinates, width, and iteration budget back to defaults. |
| **`H`** | **Toggle Waypoints Overlay** | Toggles visibility of camera waypoint beacon pins, framing boxes, and spline flight trajectories. |
...
| **`Enter`** | **Apply Coordinate / Scale Inputs** | In Center X, Center Y, Width, or Scale input fields, applies values immediately (equivalent to clicking **Set**). |
| **Zoom In / Out Buttons** | **Stepped Factor Zoom** | Multiplies or divides viewport width from screen center by the configured **Zoom Factor** value. |
...
### Row 1: Main Toolbar & Export Studio

* **Presets**:
  * Opens the preset manager to browse, load, rename, delete, import/export, and jump between curated deep-zoom coordinates. Includes an instant search filter allowing fast live filtering across bookmark titles, descriptions, and fractal types.
* **Export Studio Buttons (with Reactive Amber Busy Feedback)**:
  ...
  * **Image Zoom**: Renders sequential image frames along deep-zoom trajectories or smoothly pans and zooms across arbitrarily many waypoints recorded via double-click on the canvas. Supports session save/reload (`fractal_seq_session.json`), skipping existing rendered frames, automatic 60 FPS duration calculation, and live 256x144 calculation preview HUD.
  * **Image Mosaic**: Renders partitioned tile grids with automatic seamless stitching, memory safety diagnostics, per-tile telemetry HUD overlays, and live 256x144 calculation preview HUD.
...
* **Waypoints Overlay (`Waypoints: ON` / `Waypoints: OFF`)**:
  ...
  * **Smart Auto-Activation**: Automatically activates whenever **Image Zoom** or **Video Zoom** studio dialogs are opened, turning off when closed. Can also be toggled manually at any time via toolbar or <kbd>H</kbd>.

---

### Row 2: Formula, Iteration Budget, Antialiasing & Canvas Engine
...
* **AA Edge Tol (Antialiasing Edge Tolerance)**:
  * Configures the edge-detection derivative threshold (range: `0.001` to `2.000`, default `0.350`). Smaller values trigger supersampling on subtle color transitions; higher values restrict SSAA only to sharp boundary silhouettes.
* **Device (`GPU` / `CPU`)**:
  * Switches compute backend on the fly. `GPU` uses asynchronous NVIDIA CUDA kernels; `CPU` uses multi-threaded Numba JIT.
* **Cores (Thread Count)**:
  * Active when `CPU` backend is selected. Configures parallel worker threads (1 up to system hardware thread count).
* **Canvas (Viewport Resolution)**:
  * Selects the interactive rendering viewport resolution: `480x270`, `640x360`, `960x540`, or `1280x720`.
...
* **Min Width (Precision Mode)**:
  ...
  * **1e-24 (DD)**: 106-bit Double-Double Dekker arithmetic providing ~31 decimal digits ($10^{-24} \le w < 10^{-12}$) without arbitrary-precision performance penalties.
  * **1e-300 (Perturbation)**: Arbitrary-precision reference orbit calculation with Taylor series perturbation down to $10^{-300}$. Navigation is strictly clamped to $10^{-300}$. Operates on a tiered hierarchy using hardware FP64 above $10^{-12}$ and standard fast perturbation between $10^{-12}$ and $10^{-300}$.
...
* **Zoom Factor (Multiplier Input)**:
  * Multiplier for Zoom In/Out buttons and Right-Click zoom (range: `1.0` to `1000.0`, default: `5.0`).
* **Wheel Zoom (Multiplier Input)**:
  * Multiplier for mouse wheel zoom in / out (range: `1.001` to `1000.0`, default: `1.25`).
...
* **Navigation Control Buttons (`Set`, `Reset`, `Zoom In`, `Zoom Out`)**:
  * **Set**: Applies manual text edits in Center X, Center Y, Width, or Scale to the viewport (Shortcut: <kbd>Enter</kbd> inside any input field).
  * **Reset**: Restores factory defaults for center coordinates, width, and iteration budget (Shortcut: <kbd>R</kbd>).
  * **Zoom In**: Geometric magnification centered on the viewport scaled by the configured **Zoom Factor** multiplier.
  * **Zoom Out**: Geometric demagnification centered on the viewport scaled by the configured **Zoom Factor** multiplier.
...
### Sidebar: Real-Time Color Studio & Colormap Library

* Located on the right-hand panel:

* **Color Scheme (12 Continuous Transfer Functions)**:
  * Maps continuous normalized escape iteration values into smooth cyclic palette lookups:
...
* **🎨 Colormap Library**:
  * **Search Filter**: Instant real-time search across colormaps in the active category.
  * **Categorized Collections**: Over **1,500+ curated scientific and aesthetic colormaps** across 10 distinct collections:
    1. **★ Popular / Recommended**: Hand-picked crowd favorites (`inferno`, `magma`, `plasma`, `hot`, `viridis`, `cividis`, `twilight_shifted`, `ocean`, `coolwarm`, `cubehelix`, `rainbow`, etc.).
    2. **CartoColors**: High-clarity cartographic ramps.
    3. **cmocean**: Perceptually uniform oceanographic colormaps.
    4. **ColorBrewer2**: Renowned statistical and thematic palettes.
    5. **Cubehelix**: Intensity-monotonic spiraling color schemes.
    6. **Light & Bartlein**: Perceptually robust diverging and sequential palettes.
    7. **matplotlib**: Complete standard Matplotlib colormap collection.
    8. **MyCarta**: Linear luminance geological palettes.
    9. **Scientific**: Perceptually uniform scientific colormaps.
    10. **Tableau**: Elegant categorical and gradient palettes.
* **Instant Iteration-Cache Recoloring**: Adjusting palette phase, switching color schemes, or modulating density and contrast updates within sub-millisecond speeds (~0.08ms GPU / ~1ms CPU) using cached iteration data without re-evaluating fractal mathematics.
* **Phase Shift Spinner**:
  * Live control adjusting palette phase offset in real time with instant hardware LUT transformation. Hotkeys: <kbd>[</kbd> and <kbd>]</kbd>.
* **Density Spinbox (`0.001` to `10.000`, default `1.000`)**:
  * Controls color cycle frequency and repetition rate across all color schemes.
* **Contrast / Curve Spinbox (`0.001` to `10.000`, default `1.000`)**:
  * Controls non-linear response curves, power-law contrast, and harmonic distribution. Automatically resets to default `1.0` when switching schemes.
...
* **Image Sequence Session Save & Resume**: The Image Sequence dialog automatically saves full render metadata and waypoint sequences to `fractal_seq_session.json` in the target folder. Use **Load Session...** to restore an earlier session, and keep **Resume / Skip already rendered frames** enabled to continue interrupted sequence renders instantly.
* **Auto Frame Count Calculator**: In Image Sequence export, click **Auto (60 FPS Video Length)** to automatically compute the exact frame count matching the smooth logarithmic optical speed of a 60 fps video fly-through.
...
```
apeiron/
├── apeiron.py            # Primary desktop application entrypoint
├── main.py               # Alternative entrypoint launcher
...
├── user_bookmarks.json   # (Optional) User-saved custom coordinates and bookmarks
├── fractal_seq_session.json # (Optional) Saved image sequence export session metadata
├── requirements.txt      # Python package runtime dependencies
├── run_apeiron.bat       # 1-click Windows desktop launcher
├── LICENSE               # MIT License
├── images/               # High-resolution showcase screenshots and UI diagrams
├── math_core/
