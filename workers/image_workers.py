...
class ImageRenderWorker(BaseFractalWorker):
    ...
    def run(self):
        try:
            ...
            strip_cb = (lambda cur, tot, stage="": self.strip_progress_updated.emit(cur, tot, stage)) if self.use_strip_rendering else None
            frame_params = RenderParameters(
                ...
                num_strips=self.num_strips if self.use_strip_rendering else 1,
                strip_callback=strip_cb,
                cancel_token=lambda: self.is_cancelled
            )
            res = render_frame(frame_params)
            if self.is_cancelled or res is None:
                return
            if self.overlay_hud:
            ...
            if not self.is_cancelled:
                self.export_finished.emit(self.path, time.perf_counter() - t0)
...
class ImageSequenceRenderWorker(BaseFractalWorker):
    def __init__(self, output_dir, file_prefix, num_images, target_w, target_h, max_iter,
        ...
        if waypoints:
            target_w_val = min(Decimal(str(w[2])) for w in waypoints)
        elif start_w is not None and end_w is not None:
            target_w_val = min(Decimal(str(start_w)), Decimal(str(end_w)))
        else:
            target_w_val = Decimal(str(end_w or start_w or 1))
        ...
        if hasattr(self, 'waypoints') and self.waypoints:
            min_zoom_w = min(wp[2] for wp in self.waypoints)
        elif hasattr(self, 'start_w') and hasattr(self, 'end_w') and self.start_w is not None and self.end_w is not None:
            min_zoom_w = min(self.start_w, self.end_w)
        else:
            min_zoom_w = getattr(self, 'end_w', getattr(self, 'start_w', Decimal("1.0")))
    ...
    def run(self):
        if hasattr(self, 'waypoints') and self.waypoints:
            ...
            min_w = min(self.start_w, self.end_w)
        else:
        ...
        try:
            ...
            def process_completed_image(job):
                ...
                if self.use_strip_rendering:
                    self.strip_progress_updated.emit(0, self.num_strips, "")
            ...
            self.export_finished.emit(self.output_dir, time.perf_counter() - t0)
        except Exception as e:
            safe_queue_drain_and_terminate(writer_queue)
            self.error.emit(str(e))
