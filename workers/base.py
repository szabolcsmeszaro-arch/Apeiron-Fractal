...
class BaseFractalWorker(QThread):
    export_finished = pyqtSignal(str, float)
    error = pyqtSignal(str)
    progress_updated = pyqtSignal(int, int, float, float)
    strip_progress_updated = pyqtSignal(int, int, str)
    ...
    def __init__(self, target_w=1280, target_h=720, max_iter=1000, cmap_name="inferno", ssaa_factor=1, edge_threshold=0.35,
                 ...
                 render_params=None, preview_hud=False,
                 use_strip_rendering=False, num_strips=32, **kwargs):
        super().__init__()
        ...
        self.use_strip_rendering = bool(use_strip_rendering)
...
class BaseVideoRenderWorker(BaseFractalWorker):
    ...
    def run(self):
        ...
        try:
            ...
            self.export_finished.emit(self.output_path, time.perf_counter() - t0)
