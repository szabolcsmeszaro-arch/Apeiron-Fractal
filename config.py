...
def load_builtin_presets():
    ...
    user_bookmarks_path = os.path.join(SCRIPT_DIR, "user_bookmarks.json")
    if os.path.exists(user_bookmarks_path):
        try:
            with open(user_bookmarks_path, "r", encoding="utf-8") as f:
                user_data = json.load(f)
                user_bms = user_data.get("bookmarks", []) if isinstance(user_data, dict) else user_data
                if isinstance(user_bms, list):
                    for ub in user_bms:
                        if isinstance(ub, dict) and "center_x" in ub:
                            ub["is_builtin"] = False
                            presets.append(ub)
        except Exception:
            pass
...
def check_ram_sufficiency(*args, **kwargs):
    ...
    is_ui_prompt = False
    if len(args) >= 2:
        if hasattr(args[0], 'winId') or (args[0] is None and isinstance(args[1], (int, float))) or not isinstance(args[0], (int, float)):
            is_ui_prompt = True
