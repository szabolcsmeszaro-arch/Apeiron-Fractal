...
class BookmarkPresetsDialog(QDialog):
    ...
    def save_user_bookmarks(self):
        """Auto-persists non-builtin user bookmarks to user_bookmarks.json."""
        user_bms = [bm for bm in self.viewer.bookmarks if not bm.get("is_builtin", False)]
        user_path = os.path.join(SCRIPT_DIR, "user_bookmarks.json")
        try:
            with open(user_path, "w", encoding="utf-8") as f:
                json.dump({"version": "1.0", "bookmarks": user_bms}, f, indent=4)
        except Exception:
            pass
    ...
    def on_import_json(self):
        ...
        if path:
            if self.viewer.bookmarks:
                reply = QMessageBox.question(
                    self,
                    "Overwrite Existing Bookmarks?",
                    "Importing bookmarks will replace your current bookmarks list.\nDo you want to proceed?",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.No
                )
