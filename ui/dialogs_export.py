...
class BaseExportDialog(QDialog):
    ...
    def done(self, r):
        ...
        self.worker = None
        if w is not None:
            for sig_name in ('export_finished', 'finished', 'error', 'progress_updated', 'strip_progress_updated', 'preview_updated'):
    ...
    def create_strip_progress_ui(self, label_text="Current Image Progress (32 Strips):"):
        self.strip_check = QCheckBox("Enable Striped Rendering (32 Strips) & Intra-Image Progress")
        self.strip_check.setChecked(False)
        self.strip_check.setToolTip("Renders each image in 32 horizontal strips and displays granular progress for the currently calculating image.")

        self.intra_progress_container = QWidget()
        layout = QVBoxLayout(self.intra_progress_container)
        layout.setContentsMargins(0, 2, 0, 2)
        layout.setSpacing(4)

        self.intra_progress_label = QLabel(label_text)
        self.intra_progress_label.setStyleSheet("color: #cbd5e1; font-weight: bold;")
        layout.addWidget(self.intra_progress_label)

        self.intra_progress_bar = QProgressBar()
        self.intra_progress_bar.setRange(0, 32)
        self.intra_progress_bar.setValue(0)
        self.intra_progress_bar.setFormat("%v / %m strips (%p%)")
        self.intra_progress_bar.setTextVisible(True)
        self.intra_progress_bar.setFixedHeight(32)
        self.intra_progress_bar.setStyleSheet("""
            QProgressBar {
        ...
        """)
        layout.addWidget(self.intra_progress_bar)

        self.intra_progress_container.setVisible(False)
        self.strip_check.toggled.connect(self.on_strip_rendering_toggled)
        return self.strip_check, self.intra_progress_container

    def on_strip_rendering_toggled(self, checked):
        if hasattr(self, 'intra_progress_container') and self.intra_progress_container is not None:
            self.intra_progress_container.setVisible(checked)
        if hasattr(self, 'worker') and self.worker is not None:
            self.worker.use_strip_rendering = bool(checked)

    def on_strip_progress(self, current, total, stage=""):
        if hasattr(self, 'intra_progress_bar') and self.intra_progress_bar is not None:
            if self.intra_progress_bar.maximum() != total:
                self.intra_progress_bar.setRange(0, total)
            self.intra_progress_bar.setValue(current)
            if stage:
                self.intra_progress_bar.setFormat(f"{stage}: %v / %m strips (%p%)")
                if hasattr(self, 'intra_progress_label') and self.intra_progress_label is not None:
                    self.intra_progress_label.setText(f"Current Image Progress — {stage}:")
            else:
                self.intra_progress_bar.setFormat("%v / %m strips (%p%)")
                if hasattr(self, 'intra_progress_label') and self.intra_progress_label is not None:
                    self.intra_progress_label.setText("Current Image Progress (32 Strips):")
...
class ImageExportDialog(BaseExportDialog):
    def __init__(self, viewer, target_cx, target_cy, target_w, max_iter, cmap_name, initial_ssaa=2, **kwargs):
        super().__init__(viewer, "Save Fractal Image (PNG)", 600, 440)
        ...
        s_chk, s_box = self.create_strip_progress_ui("Image Render Progress (32 Strips):")
        self.layout.addWidget(s_chk)
        self.layout.addWidget(s_box)
    ...
    def start_save(self):
        ...
        self.worker = ImageRenderWorker(
            ...
            use_strip_rendering=self.strip_check.isChecked(),
            num_strips=32,
            **{k: v for k, v in self.params.items() if k not in ('target_cx', 'target_cy', 'target_w', 'max_iter', 'cmap_name')}
        )
        self.worker.strip_progress_updated.connect(self.on_strip_progress)
        self.worker.export_finished.connect(lambda p, el: (
            self.on_strip_progress(0, 32),
            QMessageBox.information(self, "Success", f"Saved in {format_eta_time(el)}:\n{p}"),
            self.accept()
        ))
...
class ImageSequenceExportDialog(BaseExportDialog):
    def __init__(self, viewer, target_cx, target_cy, target_w, max_iter, cmap_name, initial_ssaa=2, **kwargs):
        super().__init__(viewer, "Export Zoom / Pan Image Sequence (PNG Series)", 720, 680)
        self.params = dict(kwargs, target_cx=target_cx, target_cy=target_cy, target_w=target_w, max_iter=max_iter, cmap_name=cmap_name)
        self.waypoints = []
        ...
        s_chk, s_box = self.create_strip_progress_ui("Current Frame Progress (32 Strips):")
        self.layout.addWidget(s_chk)
        self.layout.addWidget(s_box)
    ...
    def start_export(self):
        ...
        self.worker.export_finished.connect(lambda p, el: (
            self.on_strip_progress(0, 32),
            QMessageBox.information(self, "Success", f"Rendered in {format_eta_time(el)}:\n{p}"),
            self.accept()
        ))
        self.worker.error.connect(lambda err: (
            self.on_strip_progress(0, 32),
...
class MosaicExportDialog(BaseExportDialog):
    def __init__(self, viewer, target_cx, target_cy, target_w, max_iter, cmap_name, initial_ssaa=2, **kwargs):
        super().__init__(viewer, "Export Mosaic Grid (PNG Series)", 650, 640)
        self.params = dict(kwargs, target_cx=target_cx, target_cy=target_cy, target_w=target_w, max_iter=max_iter, cmap_name=cmap_name)
        ...
        self.layout.addWidget(p_chk)
        self.layout.addWidget(p_box)
        s_chk, s_box = self.create_strip_progress_ui("Current Tile Progress (32 Strips):")
        self.layout.addWidget(s_chk)
        self.layout.addWidget(s_box)
    ...
    def start_export(self):
        ...
        self.worker.export_finished.connect(lambda p, el: (
            self.on_strip_progress(0, 32),
            QMessageBox.information(self, "Success", f"Mosaic completed in {format_eta_time(el)}:\n{p}"),
            self.accept()
        ))
        self.worker.error.connect(lambda err: (
            self.on_strip_progress(0, 32),
...
class GeneralMandelbrotMorphImageExportDialog(BaseExportDialog):
    ...
    def __init__(self, viewer, target_cx, target_cy, target_w, max_iter, cmap_name, initial_ssaa=2, **kwargs):
        super().__init__(viewer, "General Mandelbrot: k-Parameter Morph Image Sequence", 720, 700)
        self.params = dict(kwargs, target_cx=target_cx, target_cy=target_cy, target_w=target_w, max_iter=max_iter, cmap_name=cmap_name)
...
class BaseVideoExportDialog(BaseExportDialog):
    ...
    def launch_video_worker(self, worker_cls, worker_kwargs):
        ...
        self.worker.export_finished.connect(lambda p, el: (QMessageBox.information(self, "Success", f"Video exported in {format_eta_time(el)}:\n{p}"), self.accept()))
        self.worker.error.connect(lambda err: (
            self.start_btn.setEnabled(True),
